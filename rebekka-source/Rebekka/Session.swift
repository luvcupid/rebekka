...
public class Session {
    ...
    public init(configuration: SessionConfiguration, completionHandlerQueue: OperationQueue = OperationQueue.main) {
        self.operationQueue = OperationQueue()
        self.operationQueue.maxConcurrentOperationCount = 1
        self.operationQueue.name = "net.ftp.rebekka.operations.queue"
        self.streamQueue = DispatchQueue(label: "net.ftp.rebekka.cfstream.queue")
    ...
    public func upload(fileURL: URL, path: String, completionHandler: @escaping BooleanResultCompletionHandler) {
...
public struct SessionConfiguration {
    ...
    internal var url: URL? {
        return URL(string: host.hasPrefix("ftp://") ? host : "ftp://\(host)/")
    }
}
